...
function ReportDataComponent() {
    ...
    // Fetch Data
    const [reports, setReports] = useState<{ data: ReportsOut[], count: number }>({ data: [], count: 0 });
    ...
    const reportSearch = async () => {
        ...
        const { data, count }: { data: ReportsOut[], count: number } = await res.json()
        setReports({ data: data, count: count });
        setLoading(false);
    }
                                    ...
                                    <Table.HeadCell className="bg-black text-white dark:bg-white dark:text-black">
                                        <button onClick={(e) => {
                                            if (currentFilter.sortFilter != "createdAt") {
                                                setCurrentFilter({ ...currentFilter, page: 1, sortFilter: "createdAt", sortOrder: true, toggle: !currentFilter.toggle })
                                                return;
                                            }
                                            setCurrentFilter({ ...currentFilter, page: 1, sortFilter: "createdAt", sortOrder: !currentFilter.sortOrder, toggle: !currentFilter.toggle })
                                        }} className="w-full flex">
