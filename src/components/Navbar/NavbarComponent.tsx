...
function NavbarComponent({ scrollHeaderClick }: { scrollHeaderClick: () => void }) {
    const pathname = usePathname();
    if (pathname == "/login") {
        return (null);
    }
    return (
        <div className={'bg-cover' + (pathname == "/" ? (" bg-center min-h-screen") : (" bg-top min-h-fit"))} style={{ backgroundImage: "url('/assets/bgpfp.jpg')" }}>
            <div className='flex flex-grow flex-col h-full w-full backdrop-brightness-50 bg-black/30'>
                <div className={'min-h-fit flex items-center' + (pathname == "/" ? (" my-4") : (""))}>
                    <Navbar className="w-full bg-transparent md:px-[7vw] sm:px-[4vw] text-white" fluid rounded>
                        <Navbar.Brand href="/">
                            <svg width="40px" height="40px" viewBox="0 0 24 24" fill="none" stroke='white' xmlns="http://www.w3.org/2000/svg">
